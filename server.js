...
// Récupérer tous les trajets disponibles (public) ── AVEC PRIX ──
app.get("/api/client/trajets", (req, res) => {
  db.query(
    "SELECT id, depart, destination, heure, places, prix FROM trajets ORDER BY heure DESC",
    (err, results) => {
      if (err) return res.status(500).json({ message: "Erreur serveur" });
      res.json(results);
    }
...
app.get("/api/client/trajets/:id", (req, res) => {
  ...
  db.query(
    "SELECT id, depart, destination, heure, places, prix FROM trajets WHERE id = ?",
    [trajetId],
    (err, results) => {
      if (err) return res.status(500).json({ message: "Erreur serveur" });
...
// ✅ CORRECTION: prix avec p minuscule + prix dans SQL
    ...
    async (err, results) => {
      ...
      res.json({
        ...
        user: {
          ...
          telephone: user.telephone, driverId: user.driver_id,
          vehicle_type: user.vehicle_type, vehicle_plate: user.vehicle_plate, seats: user.seats
        }
      });
    }
  );
...
// ✅ CORRECTION: inclure prix dans my_trips
app.get("/api/driver/my_trips", authenticateDriver, (req, res) => {
  const { driver_id } = req.query;
  if (!driver_id) return res.status(400).json({ message: "driver_id requis" });

  db.query(
    "SELECT id, depart, destination, heure, places, prix FROM trajets WHERE user_id = (SELECT user_id FROM drivers WHERE id = ?) ORDER BY heure DESC",
    [driver_id],
    (err, results) => {
      if (err) return res.status(500).json({ message: "Erreur serveur" });
      res.json(results);
...
// ✅ CORRECTION: inclure prix dans create trip
...
app.get("/api/trips/driver_requests", authenticateDriver, (req, res) => {
  db.query(
    `SELECT dm.id, dm.depart, dm.destination, dm.places, dm.status,
...
app.post("/api/trips/reservation_action", authenticateDriver, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      res.json({ success: true, message: status === "accepted" ? "Réservation acceptée" : "Réservation refusée" });
    }
  );
});

// ✅ CORRECTION: aussi accept/reject séparés pour api.ts
app.post("/api/trips/accept_reservation", authenticateDriver, (req, res) => {
  const { reservation_id } = req.body;
  if (!reservation_id) return res.status(400).json({ message: "reservation_id requis" });

  db.query("UPDATE demandes SET status = 'accepted' WHERE id = ?", [reservation_id], (err, result) => {
    if (err) return res.status(500).json({ message: "Erreur serveur" });
    if (result.affectedRows === 0) return res.status(404).json({ message: "Réservation introuvable" });
...
app.post("/api/trips/reject_reservation", authenticateDriver, (req, res) => {
  const { reservation_id } = req.body;
  if (!reservation_id) return res.status(400).json({ message: "reservation_id requis" });

  db.query("UPDATE demandes SET status = 'rejected' WHERE id = ?", [reservation_id], (err, result) => {
    if (err) return res.status(500).json({ message: "Erreur serveur" });
    if (result.affectedRows === 0) return res.status(404).json({ message: "Réservation introuvable" });
...
// ✅ CORRECTION: route DELETE + route POST /api/trips/delete (pour api.ts)
app.post("/api/trips/delete", authenticateDriver, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      db.query("DELETE FROM trajets WHERE id = ?", [trip_id], (err2, result2) => {
...
// Garder aussi la route DELETE pour compatibilité
...
app.get("/api/transport/lignes/:id/arrets", (req, res) => {
  ...
  db.query(
    `SELECT a.*, la.ordre, la.heure_theorique
     FROM arrets_bus a JOIN ligne_arrets la ON a.id = la.arret_id
     WHERE la.ligne_id = ? ORDER BY la.ordre`,
    [ligneId],
    (err, rows) => {
      if (err) return res.status(500).json({ message: "Erreur serveur" });
      res.json(rows);
...
// ✅ CORRECTION: bloc dupliqué supprimé - une seule route itineraires
...
function findArretsProches(lat, lon, rayon) {
  return new Promise((resolve, reject) => {
    ...
    const sql = `
      ...
      WHERE latitude BETWEEN ? - ? AND ? + ?
        AND longitude BETWEEN ? - ? AND ? + ?
      HAVING distance_km < ? ORDER BY distance_km LIMIT 10
    `;
    db.query(sql, [lat, lon, lat, lat, distance, lat, distance, lon, distance, lon, distance, rayon / 1000], (err, rows) => {
      if (err) reject(err);
      else resolve(rows);
...
function getLignesByArret(arretId) {
  return new Promise((resolve, reject) => {
    db.query(
      `SELECT l.*, la.ordre FROM lignes_bus l JOIN ligne_arrets la ON l.id = la.ligne_id WHERE la.arret_id = ?`,
      [arretId],
...
function getOrdreArret(ligneId, arretId) {
  return new Promise((resolve, reject) => {
    db.query(
      ...
      [ligneId, arretId],
      (err, rows) => { if (err) reject(err); else resolve(rows.length ? rows[0].ordre : null); }
    );
  });
}
...
function getHorairesProchains(ligneId) {
  return new Promise((resolve, reject) => {
    ...
    db.query(
      ...
      [ligneId, currentTime],
      (err, rows) => { if (err) reject(err); else resolve(rows.map(r => r.heure_depart)); }
    );
  });
}
